...
wheel_def_mat.RollingFriction=0.001
wheel_def_mat.SpinningFriction=0.001
...
rule_vocab.create_rule("Remove",["G"],[],0,0,[])
...
# %%
def get_initial_mech(rule_vocabul):
    graph = GraphGrammar()
    rules = ["Init",
...
def create_track():
    ...
    floor.body.GetVisualShape(0).SetTexture(".\\chess.png", 0.0003, 0.0003)
    floor.body.GetVisualShape(0).SetOpacity(0.2)
    floor.body.SetBodyFixed(True)
    return floor
...
parameters["reverse_rotate"] = 1

scenario.run_simulation(graph, parameters, starting_positions=startings, vis = True, delay=True, is_follow_camera = True)
