...
from rostok.block_builder.basic_node_block import (Block, BlockBody,
                                                   BlockBridge, BlockTransform,
                                                   BlockType, RobotBody)
from rostok.block_builder.mesh import o3d_to_chrono_trianglemesh
...
class BoxChronoBody(ChronoBody, RobotBody):
    ...
    def __init__(self,
        ...
        box_asset.GetBoxGeometry().Size = chrono.ChVectorD(shape.width_x / 2, shape.length_y / 2,
                                                           shape.height_z / 2)
        body.AddVisualShape(box_asset)

        body.SetMass(mass)

        pos_in_marker = chrono.ChVectorD(0, -shape.length_y / 2, 0)
        pos_out_marker = chrono.ChVectorD(0, shape.length_y / 2, 0)
        super().__init__(builder,
                         body,
                         pos_in_marker,
                         pos_out_marker,
...
class MountChronoBody(ChronoBody, RobotBody):
    ...
    def __init__(self,
        ...
        box_asset = chrono.ChBoxShape()
        box_asset.GetBoxGeometry().Size = chrono.ChVectorD(width_x / 2, length_y / 2, depth_z / 2)
        body.AddVisualShape(box_asset)

        body.SetMass(mass)

        pos_input_marker = chrono.ChVectorD(0, -length_y / 2, 0)
...
class ChronoBodyEnv(ChronoBody):
    ...
    def __init__(self,
        ...
        super().__init__(builder, body, pos_in_marker, pos_out_marker, color)
