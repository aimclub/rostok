...
def criterion_calc(sim_output, weights) -> float:
    """
    Function that calculates reward for grasp device. It has four criterions. All of them should be maximized.
    1) Criterion of isotropy of contact forces, that affects on object
        Description: obj_contact_forces contains values of standard deviation of contact forces for each contact point of object.
                    If there are no contacts for object or median of contact surfaces is less than 6, criterion is equal 0.
        Otherwise, mean value of standard deviation of contact forces is calculated. We want to minimize this.
    2) Criterion of simulation time
        Description: algorithm has to maximize simulation time
    3) Criterion of fixtance between COGs
        Description: algorithm tries to minimize distance between COG of grasp object and centroid of grasp polygon
