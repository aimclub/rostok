...
class RuleVocabulary:
    ...
    def create_rule(
        ...
        if len(current_nodes) != 1:
            raise Exception(
                f"Prohibited length of the current_nodes: {len(current_nodes)}!"
            )
        ...
        if len(new_nodes) != 0:
            ...
            if (
                current_in_edges > len(new_nodes) - 1
                or current_out_edges > len(new_nodes) - 1
                or current_in_edges < 0
                or current_out_edges < 0
            ):
        ...
        for edge in new_edges:
            if (
                edge[0] > len(new_nodes) - 1
                or edge[1] > len(new_nodes) - 1
                or edge[0] < 0
                or edge[1] < 0
                or edge[0] == edge[1]
            ):
    ...
    def __str__(self):
        ...
        for rule_tule in self.rule_dict.items():
            ...
            result = (
                ...
                + rule_node.label
                + " ==> "
                + str([node[1]["Node"].label for node in rule_graph.nodes.items()])
                + " "
    ...
    def check_rules(self):
        ...
        diff = self.rules_nonterminal_node_set.difference(
            set(self.terminal_dict.keys())
        )
        if len(diff) > 0:
            print(
                f"Nodes {diff} don't have terminal rules! The set of rules is not completed!"
            )
        else:
