...
class RuleVocabulary():
    ...
    rules_nonterminal_node_set: Set(Node.label)
        the set of node labels that are used as the new nodes in the nonterminal rules.
        These are nodes that can appear in the final graph
    rules_terminal_node_set: Set(Node.label)
        the set of node labels that are used as the new nodes in the terminal rules.
        These are nodes that can appear in the final graph
    terminal_dict: dict{Node.label: [Node.label]}
        the dictionary of the terminal states of the nonterminal node
    completed: bool
        the state that changes in the check_rules() method
        if the set of rules is enough to make any graph terminal
    ...
    def create_rule(self,
        ...
        The method checks the created rule. There is no method to add already created
        rule to the vocabulary.
        ...
        Exception
            Attempt to link in or out edges of the replaced node to the node idx out of new
                subgraph length
        """
        if name in self.rule_dict:
            raise Exception('This name is already in the rule vocabulary!')

        # Currently the GraphGrammar class can only apply rules that replace one node with the new
        # system of nodes. But in future we may apply replacement of the linked set of nodes
    ...
    def check_rules(self):
        ...
        Check the rules for having at least one terminal rule for every node that appears in the
        end graph of a nonterminal rule
        """
