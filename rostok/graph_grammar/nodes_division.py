...
def sort_left_right(rob: Robot, list_S, list_B):
    """Sorting nodes of Body type into right/left (depending on finger's side) and splitting them
    by fingers
    Args:
        rob (Robot): instance of the Robot class
...
def traj_to_list(B_NODES, J_NODES, LB_NODES, RB_NODES, sim_out: dict):
    ...
    Returns:
        B: list of dictionaries that contains required information
            (id, number of contact surfaces, contact forces) about node of Body type after
            simulation
        J: list of dictionaries that contains required information (id, angle time-series values)
            about node of Joint type after simulation
        LB: list of lists that contain dictionaries with required information (id, COG)
            about node of Body type from left fingers after simulation
        RB: list of lists that contain dictionaries with required information (id, COG)
    ...
    for i in range(len(B_NODES)):
        if B_NODES[i].id == sim_out[B_NODES[i].id].id_block:
            b_temp = {
                'id': B_NODES[i].id,
                'amount_contact_surfaces': sim_out[B_NODES[i].id].amount_contact_surfaces,
                'sum_contact_forces': sim_out[B_NODES[i].id].sum_contact_forces
            }
            B.append(b_temp)
    ...
    for i in range(len(J_NODES)):
        if J_NODES[i].id == sim_out[J_NODES[i].id].id_block:
            j_temp = {
                'id': J_NODES[i].id,
                'angle_list': sim_out[J_NODES[i].id].angle_list,
                'time': sim_out[J_NODES[i].id].time
            }
            J.append(j_temp)
    ...
    for i in range(len(LB_NODES)):
        ...
        for j in range(len(LB_NODES[i])):
            if LB_NODES[i][j].id == sim_out[LB_NODES[i][j].id].id_block:
                lb_temp = {
                    'id': LB_NODES[i][j].id,
                    'abs_coord_cog': sim_out[LB_NODES[i][j].id].abs_coord_COG
                }
    ...
    for i in range(len(RB_NODES)):
        ...
        for j in range(len(RB_NODES[i])):
            if RB_NODES[i][j].id == sim_out[RB_NODES[i][j].id].id_block:
                rb_temp = {
                    'id': RB_NODES[i][j].id,
                    'abs_coord_cog': sim_out[RB_NODES[i][j].id].abs_coord_COG
                }
