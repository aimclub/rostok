...
from rostok.library.obj_grasp.objects import get_object_parametrized_sphere, get_object_easy_box, get_obj_hard_get_obj_hard_large_ellipsoid, get_object_parametrized_cylinder
from rostok.library.rule_sets.ruleset_new_style import create_rules

# create rule vocabulary
rule_vocabul = create_rules()
# create blueprint for object to grasp
grasp_object_blueprint = get_object_parametrized_cylinder(0.4, 1, 0.7)
...
while not finish:
    ...
    if current > hp.MCTS_TIMEOUT:
        break
ex = time.time() - start
print(f"time :{ex}")
print(f"Is final flag :{finish}")
...
with open(Path(path, "mcts_result_OPTI_CYLL.txt"), "a") as file:
